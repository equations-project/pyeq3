...
smallAnimationDelayBetweenFrames = int(
    ((smallAnimationModulus / largeAnimationModulus) * largeAnimationDelayBetweenFrames)
    / 2
)
...
rawData = numpy.array(
    [
        [
            numpy.array(
                [5.357, 5.797, 5.936, 6.161, 6.697, 6.731, 6.775, 8.442, 9.861]
            ),
            [],
        ],
...
def SaveModelScatterConfidence(in_fileName, in_equation, in_Ymax, in_Ymin):
    ...
    if -1 != in_fileName.find("_large"):  # large animation frame
        fig = plt.figure(
            figsize=(
                float(largeAnimationGraphWidth) / 100.0,
                float(largeAnimationGraphHeight) / 100.0,
            ),
            dpi=100,
        )
        ...
        fig = plt.figure(
            figsize=(
                float(smallAnimationGraphWidth) / 100.0,
                float(smallAnimationGraphHeight) / 100.0,
            ),
            dpi=100,
        )
...
errors in recording experimental data."""
    ...
    for i in range(0, 360):
        ...
        if not i % largeAnimationModulus:
            ...
            equation.dataCache.allDataCacheDictionary["DependentData"][-1] = (
                rawData[0][0][-1] + offset
            )
            equation.dataCache.allDataCacheDictionary["Weights"] = []
            equation.Solve()
            ...
            fileName = (
                specificBaseName + "_ci" + extraZerosString + str(i) + "_large.png"
            )
            ...
            if not (i % smallAnimationModulus):
                fileName = (
                    specificBaseName + "_ci" + extraZerosString + str(i) + "_small.png"
                )
                SaveModelScatterConfidence(fileName, equation, Ymax, Ymin)
    ...
    commandString = (
        "gifsicle --loop --colors 256 --delay "
        + str(smallAnimationDelayBetweenFrames)
        + " "
        + specificBaseName
        + "_ci*small.gif > "
        + specificBaseName
    ...
    commandString = (
        "gifsicle --loop --colors 256 --delay "
        + str(largeAnimationDelayBetweenFrames)
        + " "
        + specificBaseName
        + "_ci*large.gif > "
        + specificBaseName
    ...
    stillImageFileNameList = [
        specificBaseName + "_ci194_large.png",
    ...
    for filename in currentDir:
        ...
        if (-1 != filename.find("_ci")) and (-1 != filename.find("large.gif")):
            os.remove(filename)
        if (
            (-1 != filename.find(specificBaseName))
            and (-1 != filename.find("_large.png"))
            and (filename not in stillImageFileNameList)
        ):
            os.remove(filename)
...
errors in recording experimental data."""
    ...
    for i in range(0, 360):
        ...
        if not i % largeAnimationModulus:
            ...
            # sine wave multiplier per animation frame
            offset = numpy.sin(numpy.radians(i)) * 14.0
            equation.dataCache.allDataCacheDictionary["DependentData"][6] = (
                rawData[0][0][6] + offset
            )
            equation.dataCache.allDataCacheDictionary["Weights"] = []
            equation.Solve()
            equation.CalculateModelErrors(
    ...
    # remove unused files, saving the ones in this list
    stillImageFileNameList = [
        specificBaseName + "_ci200_large.png",
        specificBaseName + "_ci086_large.png",
        specificBaseName + "_ci270_large.png",
    ]
    currentDir = os.listdir(".")
...
when making multiple data collection runs."""
    ...
    for i in range(0, 360):
        ...
        equation.dataCache.allDataCacheDictionary["IndependentData"] = [
            numpy.append(xdata, xdata),
            numpy.ones(2 * len(xdata)),
        ]
        ...
        if not i % largeAnimationModulus:
            ...
            sys.stdout.flush()
            equation.dataCache.allDataCacheDictionary["DependentData"] = numpy.append(
                ydata + numpy.sin(numpy.radians(i)) * 4.9,
                ydata - numpy.sin(numpy.radians(i)) * 4.9,
            )
            equation.dataCache.allDataCacheDictionary["Weights"] = []
            equation.Solve()
...
data in the region that is poorly defined."""
    ...
    # remove unused files, saving the ones in this list
    stillImageFileNameList = [
        specificBaseName + "_ci022_large.png",
        specificBaseName + "_ci090_large.png",
        specificBaseName + "_ci270_large.png",
    ]
    currentDir = os.listdir(".")
...
for i in htmlInfo:
    ...
    individualFile.write(
        '<img src="'
        + i[2]
        + '_large.gif"><br><br><br>---- <b>Still Images</b> -----<br>'
    )
    for fileName in i[3]:
