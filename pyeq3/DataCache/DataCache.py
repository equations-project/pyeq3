...
class DataCache(object):
    ...
    def GenerateReducedRawData(self, inModel):
        ...
        for i in range(len(self.allDataCacheDictionary["DependentData"])):
            if (
                ...
                < self.allDataCacheDictionary["IndependentData"][0][minXindex]
            ):
                minXindex = i
            if (
                self.allDataCacheDictionary["IndependentData"][0][i]
                > self.allDataCacheDictionary["IndependentData"][0][maxXindex]
            ):
                maxXindex = i
            if inModel.GetDimensionality() == 3:
                if (
                    self.allDataCacheDictionary["IndependentData"][1][i]
                    < self.allDataCacheDictionary["IndependentData"][1][minYindex]
                ):
                    minYindex = i
                if (
                    self.allDataCacheDictionary["IndependentData"][1][i]
                    > self.allDataCacheDictionary["IndependentData"][1][maxYindex]
                ):
                    maxYindex = i
            if (
                self.allDataCacheDictionary["DependentData"][i]
                < self.allDataCacheDictionary["DependentData"][minZindex]
            ):
                minZindex = i
            if (
                self.allDataCacheDictionary["DependentData"][i]
                > self.allDataCacheDictionary["DependentData"][maxZindex]
            ):
                maxZindex = i
    ...
    def CalculateNumberOfReducedDataPoints(self, inModel):
        inModel.numberOfReducedDataPoints = (
            len(inModel.GetCoefficientDesignators()) * 3 * inModel.GetDimensionality()
        )
        # if the number of reduced data points is greater than
        # the number of all data points, use the "all data" cache
        if inModel.numberOfReducedDataPoints > len(
    ...
    def FindOrCreateReducedDataCache(self, inModel):
        ...
        return self.FindOrCreateCache_CommonCode(
            self.reducedDataCacheDictionary, inModel
        )
