...
class UserSelectableRational(pyeq3.Model_2D_BaseClass.Model_2D_BaseClass):
    ...
    def __init__(
        ...
        inRationalNumeratorFlags=[],
        inRationalDenominatorFlags=[],
        inRationalEquationList=[],
    ):
        ...
        if inRationalEquationList:
            ...
            self.rationalEquationList = (
                pyeq3.PolyFunctions.GenerateListForRationals_2D()
            )
        ...
        if 0 == (len(self.rationalNumeratorFlags) + len(self.rationalDenominatorFlags)):
            ...
            for xindex in range(len(self.rationalEquationList)):
                if xindex in self.rationalNumeratorFlags:  # numerator
                    if xindex == 0:
                        self._HTML += (
                            self.listOfAdditionalCoefficientDesignators[count] + " "
                        )
                    else:
                        self._HTML += (
                            self.listOfAdditionalCoefficientDesignators[count]
                    ...
                    if xindex == 0:
                        self._HTML += (
                            self.listOfAdditionalCoefficientDesignators[
                                count + len(self.rationalNumeratorFlags)
                            ]
                            + " "
                        )
                    else:
    ...
    def GetCoefficientDesignators(self):
        self._coefficientDesignators = list(
            self.listOfAdditionalCoefficientDesignators[
                : len(self.rationalNumeratorFlags) + len(self.rationalDenominatorFlags)
            ]
        )
    ...
    def GetDataCacheFunctions(self):
        ...
        for i in self.rationalNumeratorFlags:
            functionList.append(
                [
                    pyeq3.DataCache.DataCacheFunctions.Rational2D(
    ...
    def CalculateModelPredictions(self, inCoeffs, inDataCacheDictionary):
        ...
        try:
            for i in self.rationalNumeratorFlags:
                numerator += inCoeffs[coeffCount] * eval(
                    "inDataCacheDictionary['Rational2D_" + str(i) + "']"
                )
                coeffCount += 1
            for i in self.rationalDenominatorFlags:
                denominator += inCoeffs[coeffCount] * eval(
                    "inDataCacheDictionary['Rational2D_" + str(i) + "']"
                )
                coeffCount += 1
            ...
            if len(
                inCoeffs
            ) > coeffCount and -1 == self.extendedVersionHandler.__class__.__name__.find(
                "_Offset"
            ):
                temp += inCoeffs[-1]
    ...
    def SpecificCodeCPP(self):
        ...
        for i in self.rationalDenominatorFlags:
            if coeffCount < (
                len(self.rationalNumeratorFlags)
                + len(self.rationalDenominatorFlags)
                - 1
            ):
                if i == 0:
                    denominatorString += (
